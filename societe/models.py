# societe/models.py — VERSION CORRIGÉE (nettoyée sans modification fonctionnelle)
...
class Societe(models.Model):
    ...
    def cle_active(self):
        ...
        # 1️⃣ Chercher une clé déjà ACTIVE et valide
        cle = self.cles_activation.filter(
            statut='ACTIVE',
            utilisee=True,
            active=True,
            date_fin__gte=now,
        ...
        # 2️⃣ Sinon, chercher une clé DISPONIBLE et valide → l'activer automatiquement
        cle_dispo = self.cles_activation.filter(
            statut='DISPONIBLE',
            active=True,
            date_debut__lte=now,
            date_fin__gte=now,
        ...
        if cle_dispo:
            cle_dispo.activer()  # marque utilisee=True, statut='ACTIVE', date_utilisation=now
            return cle_dispo
