...
def rapport_facturation(request):
    ...
    for facture in qs.filter(type_facture='FN', statut_obr='ENVOYE'):
    ...
    for facture in qs.filter(type_facture='FA', statut_obr='ENVOYE'):
        devise = getattr(facture, 'devise', 'BIF') or 'BIF'
        d = totaux_par_devise[devise]
        d['total_ht']  -= quantize_3dec(facture.total_ht or 0)
        d['total_tva'] -= quantize_3dec(facture.total_tva or 0)
...
def export_facturation_excel(request):
    ...
    for facture in qs:
        ht  = quantize_3dec(getattr(facture, 'total_ht', 0))
        tva = quantize_3dec(getattr(facture, 'total_tva', 0))
        ttc = quantize_3dec(getattr(facture, 'total_ttc', 0))
    ...
    data.append(['', '', '', '', 'TOTAUX', tot_ht, tot_tva, tot_ttc, '', ''])

    titre = format_titre_avec_dates("Facturation", f['date_debut'], f['date_fin'])
    chemin = generer_excel(titre, colonnes, data, "facturation",date_debut=f['date_debut'],
        date_fin=f['date_fin'] )
    return redirect(f"/media/{chemin}")
