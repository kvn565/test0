...
class Devis(models.Model):
    ...
    numero = models.CharField(max_length=50, blank=True, editable=False, verbose_name="N° Devis")
    ...
    @property
    def display_numero(self):
        return self.numero if self.numero else "DEVIS/—"
    ...
    def generate_numero(self):
        ...
        last_seq = self.get_last_sequence()
        new_seq = last_seq + 1
        self.numero = f"DEVIS/{new_seq}/{year}"
    ...
    def save(self, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            if not self.date_validite:
                self.date_validite = (self.date_devis + timezone.timedelta(days=30)) if self.date_devis else (timezone.now().date() + timezone.timedelta(days=30))
            super().save(*args, **kwargs)
