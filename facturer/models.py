...
class Facture(models.Model):
    ...
    def generate_invoice_identifier(self):
        ...
        # Date/heure de l'envoi (régénéré dans envoyer_facture_obr avant le payload)
    ...
    def delete(self, *args, **kwargs):
        self.nettoyer_mouvements_stock()
...
class LigneFacture(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if taux == 0:
            ...
            # Calcul propre : on passe par le HTVA en premier
            montant_ht  = (prix_tvac * qte / (Decimal('1') + taux / Decimal('100'))
        ...
        # On stocke les valeurs calculées
        self.prix_unitaire_ht = (
            montant_ht / qte if qte != 0 else Decimal('0.0000')
    ...
    def _calculer_montant_ht(self):
        ...
        # Même formule que save() : calcul direct pour éviter la double troncature
        return (prix * qte / (Decimal('1') + taux / Decimal('100'))
                ).quantize(Decimal('0.001'), rounding=ROUND_DOWN)
    ...
    def nettoyer_mouvements_stock(self):
        """Nettoie les mouvements de stock EN_ATTENTE liés à cette ligne uniquement."""
        from stock.models import SortieStock, EntreeStock

        type_facture = self.facture.type_facture

        with transaction.atomic():
            if type_facture == 'FN' and self.produit:
                deleted = SortieStock.objects.filter(
                    facture=self.facture,
                    entree_stock__produit=self.produit,
                    statut_obr='EN_ATTENTE',
                ).delete()[0]
                action = f"{deleted} sortie(s) supprimée(s)"

            elif type_facture == 'FA' and self.produit:
                deleted = EntreeStock.objects.filter(
                    facture=self.facture,
