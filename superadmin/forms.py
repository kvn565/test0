...
class SocieteGeranceForm(forms.ModelForm):
    ...
    smtp_email = forms.EmailField(
        label="Email SMTP (expéditeur)",
        required=False,
        widget=forms.EmailInput(attrs={
            'class': 'form-control',
            'placeholder': 'votre.societe@gmail.com'
        }),
        help_text="Adresse Gmail utilisée pour envoyer les emails."
    )
    ...
    def save(self, commit=True):
        ...
        if not password and self.instance.pk and hasattr(instance, 'smtp_password'):
