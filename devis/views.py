...
from .forms import DevisHeaderForm, LigneDevisForm, EmailDevisForm
from .utils import envoyer_devis_email
...
def _check_droit(request):
    ...
    if not has_right:
        return None, "Vous n'avez pas les droits pour accéder aux proformas."
    return societe, None
...
def devis_liste(request):
    ...
    return render(request, 'devis/liste.html', {
        'devis': devis_page,
        'page_obj': devis_page,
        'stats': stats,
...
def devis_supprimer(request, pk):
    ...
    with transaction.atomic():
        ...
        devis.delete()
    messages.success(request, f"Proforma {num} supprimé avec succès.")
    return redirect('devis:liste')
...
def ajax_creer_devis(request):
    ...
    try:
        with transaction.atomic():
            ...
            devis.save()
        return JsonResponse({'ok': True, 'devis_id': devis.pk, 'message': 'Proforma créé'})
...
def ajax_ajouter_ligne_devis(request):
    ...
    try:
        with transaction.atomic():
            ...
            ligne = LigneDevis.objects.create(
                ...
                prix_vente_tvac=prix_ttc, quantite=quantite,
                produit=produit, service=service,
            )
    ...
    return JsonResponse({
        'ok': True, 'ligne_id': ligne.pk, 'designation': designation,
        'quantite': float(quantite), 'prix_ttc': float(prix_ttc),
...
def ajax_modifier_ligne_devis(request):
    ...
    try:
        with transaction.atomic():
            ligne.quantite = nouvelle_qte.quantize(Decimal('0.001'))
    ...
    return JsonResponse({
        'ok': True, 'ligne_id': ligne.pk, 'designation': ligne.designation,
        'quantite': float(nouvelle_qte), 'prix_ttc': float(ligne.prix_vente_tvac),
        'total_ht': float(devis.total_ht or 0),
        'total_tva': float(devis.total_tva or 0),
        'total_ttc': float(devis.total_ttc or 0),
...
def devis_valider(request, pk):
    ...
    if devis.lignes_devis.count() == 0:
        messages.error(request, "Impossible de valider un proforma vide.")
        return redirect('devis:detail', pk=pk)

    devis.statut = 'EN_ATTENTE'
    devis.save(update_fields=['statut'])
    messages.success(request, f"Proforma {devis.display_numero} validé et envoyé au client.")
    return redirect('devis:detail', pk=pk)
    ...
    try:
        ...
        with transaction.atomic():
            facture = Facture.objects.create(
                societe=societe, client=devis.client,
                devise=devis.devise, applique_tva=devis.applique_tva,
                type_facture='FN', statut_obr='EN_ATTENTE',
                cree_par=request.user,
            )

            for ligne in devis.lignes_devis.select_related('produit', 'service', 'taux_tva').all():
                lf = LigneFacture.objects.create(
                    facture=facture, produit=ligne.produit, service=ligne.service,
                    designation=ligne.designation, quantite=ligne.quantite,
                    prix_vente_tvac=ligne.prix_vente_tvac, taux_tva=ligne.taux_tva,
                )
                if ligne.produit:
                    ligne.produit.ajuster_stock(quantite=ligne.quantite, type_facture='FN', facture=facture)
        ...
        messages.success(request,
            f"Proforma {devis.display_numero} transformé en facture "
            f"<a href='{facture_url}' class='alert-link'>{facture.display_numero}</a>."
        )
        return redirect('devis:detail', pk=pk)
...
def devis_imprimer(request, pk):
    ...
    try:
        ...
        montant_lettres = num2words(int(round(devis.total_ttc or 0)), lang='fr').capitalize() + f" {devis.devise.lower()}."
    except Exception:
        montant_lettres = f"{devis.total_ttc or 0} {devis.devise}"

    context = {
        'devis': devis, 'lignes': lignes,
        'societe': societe, 'montant_lettres': montant_lettres,
        'now': timezone.now(),
    }
    ...
    try:
        ...
        response['Content-Disposition'] = f'inline; filename="devis_{devis.pk}.pdf"'
        return response
...
@login_required
@require_POST
def devis_envoyer_email(request, pk):
    societe, err = _check_droit(request)
    if err:
        messages.error(request, err)
        return redirect('accueil')
    ...
    email_dest = form.cleaned_data['email_destinataire']
    message = form.cleaned_data['message']

    success, error = envoyer_devis_email(devis, email_dest, message, request)

    if success:
        messages.success(request, f"Proforma {devis.display_numero} envoyé à {email_dest}.")
    else:
        messages.error(request, f"Erreur d'envoi : {error}")

    return redirect('devis:detail', pk=pk)


@require_http_methods(["GET"])
def devis_public(request, uuid):
    devis = get_object_or_404(Devis, uuid_public=uuid)
    lignes = devis.lignes_devis.select_related('produit', 'service').all()

    return render(request, 'devis/public.html', {
        'devis': devis,
        'lignes': lignes,
